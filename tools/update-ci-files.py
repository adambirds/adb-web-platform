...
def get_job_for_updates(ci_filename: str) -> Optional[str]:
...
def update_install_and_path_in_job(lines: List[str], frontends: List[str], job_name: str, ci_filename: str) -> List[str]:
    ...
    while i < len(lines):
        ...
        if in_target_job and re.search(r'- name:\s*Install root Node (deps|dependencies)', line):
            ...
            while i < len(lines):
                ...
                if re.search(r'- name:\s*Install .+ Node (deps|dependencies)', line_to_check) and 'root' not in line_to_check.lower():
        ...
        if in_target_job and re.search(r'- name:\s*Add node bin paths to PATH', line):
            ...
            if i < len(lines):
                ...
                if indent:
                    result.append(f'{indent}echo "$GITHUB_WORKSPACE/node_modules/.bin" >> $GITHUB_PATH\n')
                    for frontend in sorted(frontends):
                        result.append(f'{indent}echo "$GITHUB_WORKSPACE/{frontend}/node_modules/.bin" >> $GITHUB_PATH\n')
